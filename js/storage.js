...
window.KIRIS_STORAGE_CONFIG = window.KIRIS_STORAGE_CONFIG || {
  ...
  claveToken: "kirisV2_github_token",
};
...
window.KirisStorage = (() => {
  ...
  async function cargarPublicado() {
    ...
    if (!respuesta.ok) {
      throw new Error(
        `No fue posible leer ${config.archivoPublicado} (${respuesta.status}).`,
      );
    }
    ...
    if (!publicado || typeof publicado !== "object") {
      throw new Error(
        `${config.archivoPublicado} no contiene un objeto JSON válido.`,
      );
    }
  ...
  function obtenerTokenGuardado() {
    return (
      localStorage.getItem(config.claveToken) ||
      localStorage.getItem("peToken") ||
      ""
    ).trim();
  }
  ...
  function solicitarToken() {
    const token = window.prompt(
      "Pegue el token de GitHub. Se guardará únicamente en este navegador para las próximas publicaciones.",
    );
    ...
    localStorage.setItem(config.claveToken, limpio);
    return limpio;
  }

  function obtenerToken() {
    return obtenerTokenGuardado() || solicitarToken();
  }

  function olvidarToken() {
    localStorage.removeItem(config.claveToken);
  }
  ...
  async function obtenerArchivoGitHub(token) {
    ...
    if (!respuesta.ok) {
      ...
      throw new Error(
        `GitHub rechazó la lectura (${respuesta.status}). ${detalle}`,
      );
    }
  ...
  async function publicar(paquete) {
    ...
    try {
      const archivoActual = await obtenerArchivoGitHub(token);
      ...
      const cuerpo = {
        message: `Guardar y publicar datos KIRIS ${new Date().toISOString()}`,
      ...
      if (!respuesta.ok) {
        ...
        if (respuesta.status === 401 || respuesta.status === 403) {
          olvidarToken();
          throw new Error(
            "GitHub rechazó el token. Se eliminó el token guardado para solicitar uno válido en la próxima publicación.",
          );
        }

        throw new Error(
          `GitHub rechazó la publicación (${respuesta.status}). ${detalle}`,
        );
      }

      return respuesta.json();
    } catch (error) {
      if (/401|403|token/i.test(String(error?.message || ""))) {
        olvidarToken();
      }
      throw error;
    }
