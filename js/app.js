...
async function entrar(modo) { 
    ...
    try { 
        if (editorActivo) { 
            const borrador = await window.KirisStorage.cargar(); 
            if (borrador && typeof borrador === "object") estado = { ...estadoInicial(), ...borrador }; 
        } else { 
            const publicado = await window.KirisStorage.cargarPublicado(); 
            estado = { ...estadoInicial(), ...publicado, modo: "visitante" }; 
        } 
    } catch (error) { 
        console.error(error); 
    ...
    renderTodo(); 
    const estadoTexto = $("estadoSincronizacion"); 
    if (estadoTexto) estadoTexto.textContent = editorActivo ? "Borrador local" : "Última versión publicada"; 
} 
...
async function publicarCambios() { 
    const boton = $("btnPublicarCambios"); 
    ...
    try { 
        ...
        mostrarToast("Versión publicada en GitHub. El visor ya puede cargarla."); 
    } catch (error) { 
        console.error(error); 
        mostrarToast(error.message); 
    } finally { 
