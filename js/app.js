...
const COLUMNAS_TRAMITES = [ 
    ...
    { key: "listo", label: "Listo", width: 230, tipo: "select", opciones: ["", "PENDIENTE", "PENDIENTE / NO SE VE EL CAMBIO", "PENDIENTE / FALTA INFORMACIÓN", "PENDIENTE DE PUBLICAR / LISTA LA ACTUALIZACIÓN", "SOLO ESPAÃ‘OL / NO APLICA INGLÉS", "SOLO ESPAÃ‘OL / FALTA INGLÉS", "SOLO INGLÉS", "AMBOS IDIOMAS"] }, 
    { key: "acciones", label: "Acciones", width: 110, especial: "acciones" } 
]; 
...
/* ===== Mejoras integradas KIRIS ===== */ 
let filtrosSeleccion = { manuales:{}, tramites:{}, versiones:{} }; 
function columnasPorTipo(tipo){ return tipo === "manuales" ? COLUMNAS_MANUALES : tipo === "tramites" ? COLUMNAS_TRAMITES : COLUMNAS_VERSIONES; } 
function renderSegunTipo(tipo){ if(tipo==="manuales")renderManuales(); if(tipo==="tramites")renderTramites(); if(tipo==="versiones")renderVersiones(); } 
function entrar(){ estado.modo="editor"; editorActivo=true; document.body.classList.remove("modo-visitante"); $("loginScreen").hidden=true; $("app").hidden=false; const b=$("modoUsuarioBadge"); if(b){b.textContent="Editor";b.className="modo-badge-editor";} renderTodo(); } 
function configurarLogin(){ $("btnIngresarEditor")?.addEventListener("click",entrar); $("btnCerrarSesion")?.addEventListener("click",cerrarSesion); } 
function cerrarFiltroExcel(){ const m=$("filterMenu"); if(m){m.hidden=true;m.innerHTML="";} } 
function abrirFiltroExcel(tipo,key,ancla){ const col=columnasPorTipo(tipo).find(c=>c.key===key); if(!col)return; const valores=[...new Set((estado[tipo]||[]).map(x=>String(valorVisible(x,col)??"")))].sort((a,b)=>a.localeCompare(b,"es",{numeric:true})); const activos=filtrosSeleccion[tipo][key]||[],m=$("filterMenu"); m.innerHTML=`<div class="excel-filter-title">${escaparHTML(col.label)}</div><input id="excelFilterSearch" class="filter-input" placeholder="Buscar valores..."><label class="excel-filter-option"><input id="excelFilterAll" type="checkbox" ${!activos.length||activos.length===valores.length?"checked":""}> Seleccionar todo</label><div id="excelFilterValues" class="excel-filter-values">${valores.map(v=>`<label class="excel-filter-option" data-value-text="${escaparHTML(normalizar(v))}"><input type="checkbox" value="${escaparHTML(v)}" ${!activos.length||activos.includes(v)?"checked":""}> ${escaparHTML(v||"(Vacío)")}</label>`).join("")}</div><div class="excel-filter-actions"><button id="excelFilterClear" class="btn btn-light">Borrar filtro</button><button id="excelFilterApply" class="btn btn-primary">Aplicar</button></div>`; const r=ancla.getBoundingClientRect();m.style.left=`${Math.max(8,Math.min(r.left,innerWidth-330))}px`;m.style.top=`${Math.max(8,Math.min(r.bottom+4,innerHeight-430))}px`;m.hidden=false; $("excelFilterSearch").oninput=e=>m.querySelectorAll("[data-value-text]").forEach(l=>l.hidden=!l.dataset.valueText.includes(normalizar(e.target.value))); $("excelFilterAll").onchange=e=>m.querySelectorAll('#excelFilterValues input[type="checkbox"]').forEach(c=>{if(!c.closest("label").hidden)c.checked=e.target.checked}); $("excelFilterClear").onclick=()=>{delete filtrosSeleccion[tipo][key];cerrarFiltroExcel();renderSegunTipo(tipo)}; $("excelFilterApply").onclick=()=>{const a=[...m.querySelectorAll('#excelFilterValues input[type="checkbox"]:checked')].map(c=>c.value);filtrosSeleccion[tipo][key]=a.length===valores.length?[]:a;cerrarFiltroExcel();renderSegunTipo(tipo)}; } 
function cumpleFiltros(objeto,tipo,columnas){ return columnas.every(col=>{if(col.especial)return true;const v=String(valorVisible(objeto,col)??""),q=filtros[tipo][col.key],a=filtrosSeleccion[tipo][col.key];return(!q||normalizar(v).includes(normalizar(q)))&&(!Array.isArray(a)||!a.length||a.includes(v));}); } 
function habilitarMovimientoColumnas(el,tipo){el.querySelectorAll('th[draggable="true"]').forEach(th=>{th.ondragstart=e=>{if(e.target.closest("input,button,.resize-handle")){e.preventDefault();return}e.dataTransfer.setData("text/plain",th.dataset.key);th.classList.add("columna-arrastrando")};th.ondragend=()=>th.classList.remove("columna-arrastrando");th.ondragover=e=>{e.preventDefault();th.classList.add("columna-destino")};th.ondragleave=()=>th.classList.remove("columna-destino");th.ondrop=e=>{e.preventDefault();th.classList.remove("columna-destino");const cols=columnasPorTipo(tipo),a=e.dataTransfer.getData("text/plain"),b=th.dataset.key,i=cols.findIndex(c=>c.key===a),j=cols.findIndex(c=>c.key===b);if(i>=0&&j>=0&&i!==j){cols.splice(j,0,cols.splice(i,1)[0]);renderSegunTipo(tipo)}}})} 
function crearEncabezado(elemento,columnas,tipo,ocultas=[]){elemento.innerHTML=`<tr>${columnas.map(col=>{const st=ocultas.includes(col.key)?"display:none":"";if(col.especial==="seleccion")return`<th style="${st}"><input id="seleccionarTodos_${tipo}" type="checkbox"></th>`;if(col.especial)return`<th data-key="${col.key}" draggable="true" style="${st}"><div class="th-content"><span>${escaparHTML(col.label)}</span><span class="resize-handle" data-tipo="${tipo}" data-key="${col.key}"></span></div></th>`;return`<th data-key="${col.key}" draggable="true" style="${st}"><div class="th-content"><span>${escaparHTML(col.label)}</span><span class="resize-handle" data-tipo="${tipo}" data-key="${col.key}"></span></div><div class="filter-combo"><input class="filter-input" data-tipo="${tipo}" data-key="${col.key}" value="${escaparHTML(filtros[tipo][col.key]||"")}" placeholder="Buscar o filtrar..."><button class="excel-filter-btn" data-key="${col.key}" type="button">▼</button></div></th>`}).join("")}</tr>`;elemento.querySelectorAll(".filter-input").forEach(i=>{i.onclick=e=>{e.stopPropagation();abrirFiltroExcel(tipo,i.dataset.key,i)};i.oninput=()=>{filtros[tipo][i.dataset.key]=i.value;renderSegunTipo(tipo);const n=document.querySelector(`.filter-input[data-tipo="${tipo}"][data-key="${i.dataset.key}"]`);n?.focus();n?.setSelectionRange(n.value.length,n.value.length)}});elemento.querySelectorAll(".excel-filter-btn").forEach(b=>b.onclick=e=>{e.stopPropagation();abrirFiltroExcel(tipo,b.dataset.key,b)});habilitarMovimientoColumnas(elemento,tipo);} 
function horasTotalesRegistro(r){return estado.bitacora.filter(x=>normalizar(x.manual)===normalizar(r.manual)).reduce((t,x)=>t+Number(x.horas||0),0)} 
function resumenBitacora(r){return[`Manual: ${r.manual||""}`,`Tipo: ${r.tipo||""}`,`Hora inicio: ${r.horaInicio||""}`,`Hora fin: ${r.horaFin||""}`,`Horas: ${Number(r.horas||0).toFixed(2)}`,`Páginas: ${r.paginas??""}`,`Horas totales: ${horasTotalesRegistro(r).toFixed(2)}`,`Detalle: ${r.detalle||""}`].join("\n")} 
const renderBitacoraBase=renderBitacora; renderBitacora=function(){renderBitacoraBase();document.querySelectorAll("[data-registro-id]").forEach(e=>{const r=estado.bitacora.find(x=>x.id===e.dataset.registroId);if(r)e.title=resumenBitacora(r)})}; 
function eliminarSeleccionados(tipo){const singular=tipo==="manuales"?"manual":tipo==="tramites"?"tramite":"version",ids=[...document.querySelectorAll(`.seleccion-${singular}:checked`)].map(c=>c.dataset.id);if(!ids.length)return mostrarToast("No hay registros seleccionados");if(!confirm(`¿Eliminar ${ids.length} registro(s)?`))return;estado[tipo]=estado[tipo].filter(x=>!ids.includes(x.id));guardarEstado("Registros eliminados");renderSegunTipo(tipo)} 
function detectarSistemaVersion(texto){return normalizar(texto).includes("siscard+")?"siscard+":"SISCARD"} 
async function importarExcelVersiones(archivo){if(!archivo)return;try{if(typeof XLSX==="undefined")throw new Error("No se cargó XLSX");const reemplazar=confirm("¿Desea reemplazar el Control de Versiones actual?\nAceptar = Reemplazar.\nCancelar = Agregar al final."),w=XLSX.read(await archivo.arrayBuffer(),{type:"array"}),n=[];w.SheetNames.forEach(nombre=>{const nn=normalizar(nombre),idioma=nn.includes("ingles")||nn.includes("english")?"Inglés":nn.includes("espanol")||nn.includes("spanish")?"Español":"";if(!idioma)return;XLSX.utils.sheet_to_json(w.Sheets[nombre],{header:1,defval:""}).forEach(f=>{const codigo=String(f[0]||"").trim(),manual=String(f[1]||"").trim(),numero=String(f[2]??"").trim()||"0";if(!codigo.startsWith("UEO-")||!manual)return;n.push({id:id("version"),sistema:detectarSistemaVersion(manual),codigo,manual,idioma,numero,fecha:"",estado:"Disponible",observaciones:""})})});if(!n.length)throw new Error("No se encontraron registros válidos");estado.versiones=reemplazar?n:[...estado.versiones,...n];guardarEstado(`${n.length} versiones importadas`);renderVersiones()}catch(e){mostrarToast(`No fue posible importar: ${e.message}`)}finally{$("inputExcelVersiones").value=""}} 
function exportarExcelVersiones(){const datos=estado.versiones.map(v=>({"Sistema":v.sistema||"","Código":v.codigo||"","Manual":v.manual||"","Idioma":v.idioma||"","Versión disponible":v.numero||"","Fecha de versión":v.fecha||"","Estado":v.estado||"","Observaciones":v.observaciones||""})),w=XLSX.utils.book_new(),hoja=XLSX.utils.json_to_sheet(datos);XLSX.utils.book_append_sheet(w,hoja,"Control Versiones");XLSX.writeFile(w,`Control_Versiones_${fechaISOHoy()}.xlsx`)} 
const configurarBotonesBase=configurarBotones; configurarBotones=function(){configurarBotonesBase();const imp=$("inputExcelVersiones"),nuevoImp=imp.cloneNode(true);imp.parentNode.replaceChild(nuevoImp,imp);nuevoImp.onchange=e=>importarExcelVersiones(e.target.files[0]);const exp=$("btnExportarVersiones"),nuevoExp=exp.cloneNode(true);exp.parentNode.replaceChild(nuevoExp,exp);nuevoExp.onclick=exportarExcelVersiones;[["btnLimpiarFiltrosManuales","manuales"],["btnLimpiarFiltrosTramites","tramites"],["btnLimpiarFiltrosVersiones","versiones"]].forEach(([b,t])=>$(b).addEventListener("click",()=>filtrosSeleccion[t]={}));}; 
document.addEventListener("click",e=>{const m=$("filterMenu");if(m&&!m.hidden&&!m.contains(e.target)&&!e.target.closest(".filter-combo"))cerrarFiltroExcel()}); 
