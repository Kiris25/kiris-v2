...
function crearEncabezado(elemento,columnas,tipo,ocultas=[]){const titulos=columnas.map(c=>{const o=ocultas.includes(c.key)?"display:none":"";if(c.especial==="seleccion")return `<th style="${o}"><input id="seleccionarTodos_${tipo}" type="checkbox" aria-label="Seleccionar todos"></th>`;return `<th data-key="${c.key}" style="${o}"><div class="th-content"><span>${escaparHTML(c.label)}</span><span class="resize-handle" data-tipo="${tipo}" data-key="${c.key}"></span></div></th>`;}).join("");const filtrosHtml=columnas.map(c=>{const o=ocultas.includes(c.key)?"display:none":"";if(c.especial)return `<th style="${o}"></th>`;const cfg=filtros[tipo][c.key],texto=typeof cfg==="object"?(cfg.texto||""):(cfg||""),activo=cfg&&(texto||(cfg.valores||[]).length);return `<th style="${o}"><input class="filter-input ${activo?"filtro-activo":""}" data-tipo="${tipo}" data-key="${c.key}" value="${escaparHTML(texto)}" placeholder="Buscar o filtrar"></th>`;}).join("");elemento.innerHTML=`<tr>${titulos}</tr><tr class="filters-row">${filtrosHtml}</tr>`;elemento.querySelectorAll(".filter-input").forEach(input=>{const col=columnas.find(c=>c.key===input.dataset.key);input.oninput=()=>{const prev=filtros[tipo][col.key];filtros[tipo][col.key]={texto:input.value,valores:typeof prev==="object"?(prev.valores||[]):[]};renderEntidad(tipo);const n=document.querySelector(`.filter-input[data-tipo="${tipo}"][data-key="${col.key}"]`);n?.focus();n?.setSelectionRange(n.value.length,n.value.length);};input.onclick=e=>abrirMenuFiltroKiris(e,tipo,col,input);});} 
document.addEventListener("click",cerrarMenuFiltroKiris); 
